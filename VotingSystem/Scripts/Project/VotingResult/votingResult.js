...
function showContentOfPage(pageNum) {
    ...
    $.post("/VotingResult/GetResultList", content,
        function (res) {
            if(res.code === 200){
                var data = res.list;
                var showHtml = "";
                ...
                for(var i in data){
                    ...
                    "<div class='weui-cells weui-cells_form'>" +
                        "<div class='weui-cell'>" +
                            "<div class='weui-cell__hd'><label class='weui-label'>结果</label></div>"+
                ...
                $('#result_pageTotal').text("共有"+totalPageNum+"页,当前第"+pageNum+"页");
...
// 监听“上一页”按钮
$(document).on("click", "#result_lastPage", function() {
    if(page === 1){
        $.toast("当前已是第一页！");
    }else{
        page--;
...
// 监听“下一页”按钮
$(document).on("click", "#result_nextPage", function() {
